...
def find_crackwidth_point(crack_frame, frame, points, scale=1, show=True):
...
if __name__ == '__main__':
    ...
    rot_frame = find_compaction(rot_frame, frame_index, show=True)


    writevid = WriteVideo(filename=filename[:-4] + 'annotation.mp4',frame_size=np.shape(rot_frame))

    output = [[0,cracktip_x*scale, R*scale]]
    for index in range(700):
        ...
        rot_frame, R = find_cracktip_radius(crack_frame, rot_frame, cracktip_x)
        if index%50 == 0:
            rot_frame = find_compaction(rot_frame, index, show=True)
        else:
            rot_frame = find_compaction(rot_frame, index, show=False)

        writevid.add_frame(rot_frame)
        #frame(rot_frame)
        output.append([index*0.2,cracktip_x*scale, R*scale])
    writevid.close()
    np.savetxt(filename[:-4] + 'crackcurvature.txt',np.array(output))
