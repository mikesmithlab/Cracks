...
def find_cracktip(crack_frame,frame, minx=400, threshold=10, behind_tip=400, show=False):
...
def find_cracktip_radius(crack_frame, rot_frame, cracktip_x, min_trail=10, max_trail=100, show=False):
    width = []
...
if __name__ == '__main__':
    ...
    pos= find_compaction(rot_frame, frame_index)
    ...
    for index in range(700):
        ...
        cracktip_x, cracktip_y, rot_frame = find_cracktip(crack_frame,
                                                          rot_frame)
        compact_pos = find_compaction(rot_frame, index)

        time = (index+1)/fps
        output.append([time, compact_pos*scale, cracktip_x*scale])

    np.savetxt(filename[:-4] + 'poscracktip.txt',np.array(output))
